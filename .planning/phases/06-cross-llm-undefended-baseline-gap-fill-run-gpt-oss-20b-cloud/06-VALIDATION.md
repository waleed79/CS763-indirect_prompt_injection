...
| P6-D12-FUSED | `figures/d12_cross_model_heatmap_v6.png` exists, size > 1KB after Wave 4 renderer runs (renderer internally enforces `assert matrix.shape == (5, 5)` — not directly mocked at unit level) | unit (happy-path file existence + size > 1KB; shape assertion validated indirectly via renderer self-check) | `pytest tests/test_phase6_eval.py::TestD12FusedV6 -x` | ❌ W0 | ⬜ pending |
| P6-D12-UND | `figures/d12_undefended_v6.png` exists, size > 1KB after Wave 4 renderer runs (renderer internally enforces `assert matrix.shape == (5, 4)` — not directly mocked at unit level) | unit (happy-path file existence + size > 1KB) | `pytest tests/test_phase6_eval.py::TestD12UndefendedV6 -x` | ❌ W0 | ⬜ pending |
| P6-D03 | `figures/d03_arms_race_v6.png` exists, size > 1KB after Wave 4 renderer runs (size delta vs original is informational only — verified by Plan 05 stub, not unit-mocked) | smoke (happy-path file existence + size > 1KB) | `pytest tests/test_phase6_eval.py::TestD03V6 -x` | ❌ W0 | ⬜ pending |
